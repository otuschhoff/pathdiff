...
The receiver selects a protocol for each new connection: `{` starts JSON lines, `<` starts raw XML notifications, and a 4-byte big-endian length-prefixed connection is classified from its first handshake payload as XML or protobuf. ONTAP's native XML transport, identified from a recorded session, uses a `0x22` message-type byte, a 4-byte big-endian length, an XML header/body payload separated by a blank line, and a trailing NUL byte. It sends `NEGO_REQ`; `pathdiff` replies with a matching `NEGO_RESP` that selects protocol `1.0` and reports `SUCCESS`.
