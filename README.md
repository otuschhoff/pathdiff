...
The receiver selects a protocol for each new connection: `{` starts JSON lines, `<` starts raw XML notifications, and a 4-byte big-endian length-prefixed connection is classified from its first handshake payload as XML or protobuf. ONTAP's native XML transport, identified from a recorded session, uses a `0x22` message-type byte, a 4-byte big-endian length, an XML header/body payload separated by a blank line, and a trailing NUL byte. It sends `NEGO_REQ`; `pathdiff` replies with the reference-compatible `NEGO_RESP` layout: `HandshakeResp` containing the request's `VsUUID`, `PolicyName`, `SessionId`, and scalar protocol version `1.2`. Header-only native messages such as `KEEP_ALIVE` have `ContentLen` `0` and no separator; `pathdiff` accepts and ignores them while keeping the session open.

XML notifications accept `file_id`/`inode`, `path`, `operation`, and `timestamp_nsec` as elements or attributes. Protobuf decodes the supplied `netapp.fpolicy.FileOperationNotification` layout: header timestamp, inode, path, and operation. A generic framed session's first frame is treated as its handshake request and receives an empty framed acknowledgement before notification streaming begins.
