...
Use `Config.Database` to supply an already-open `Database`; in that mode the caller retains ownership and must close it. `SetListeners` supports runtime endpoint reconciliation and persists the result, and `Config.RestoreListeners` reopens that persisted set at `Start` before discovery runs. `Config.Refresh` can delegate application-specific discovery to a callback, which the receiver invokes in the background at startup and then at `RefreshInterval`. `Database.Senders` returns the last known state of every observed sender. cDOT SSH discovery, policy activation, systemd management, and table rendering remain CLI concerns under `cmd/pathdiff`.
...
`service status` renders the systemd state, active FPolicy connection count, registered event count, and average accepted FPolicy requests per second since daemon start. The daemon discovers cDOT external engines that target its local IPv4 addresses, listens only on their configured ports, and accepts each port only from LIF IPv4s on the policy's SVM. It persists that endpoint configuration in Pebble and reopens it immediately at startup, so a restart resumes accepting senders without waiting for SSH; cDOT discovery then runs in the background and reconciles any drift. Verification of the cDOT configuration (SVM, volume, LIF, and node data) repeats every 15 minutes, adds newly configured senders, attempts to enable their policies, removes ports that are no longer configured, and refreshes the cached LIF-to-SVM/node and volume mappings. Run `service refresh` to request the same cDOT reconciliation immediately; `service list-ports` shows the currently active ports and their allowed LIF IPv4 addresses. Before `cdot fpolicy start` enables and connects policies, it requests this refresh so their target ports are listening first. The internal `daemon run` entrypoint handles SIGINT and SIGTERM by stopping listeners, closing active sockets, waiting for workers, then closing Pebble.
...
The daemon accepts one JSON event per TCP line, validates that each event contains a path, and writes it through a single Pebble batch. Each receiver connection stores events concurrently, independently of control requests. Expensive index scans use a bounded admission pool sized from available CPU capacity, while event writes and lightweight control actions bypass that pool. Event queries load volume and SVM mappings once per scan rather than issuing extra Pebble reads for every result.

Each event is indexed twice: a timestamp-first key supports ordered monitoring queries, while a path-first key supports prefix and time-window audit queries. Both keys include the same process-unique sequence, so otherwise identical notifications remain separate records, and the two index updates remain atomic. Existing databases with legacy unsuffixed keys remain readable and eligible for retention cleanup. The local Unix control socket keeps database ownership in the daemon; all client commands query or control it through a compact JSON request/response protocol.
