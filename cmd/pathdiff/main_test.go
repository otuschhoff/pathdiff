...
func TestFramedXMLSession(t *testing.T) {
	...
	notification := []byte(`<FPolicy xmlns="http://www.netapp.com/fpolicy"><Header><SeqNum>5001</SeqNum><SessionID>sess-998811</SessionID></Header><NotificationRequest><Vserver>svm-50</Vserver><FileId>1048576</FileId><VolumeUuid>98765432-abcd-ef01-2345-6789abcdef01</VolumeUuid><Path>/vol/data/engineering/main.go</Path><Operation>WRITE</Operation><Timestamp>1724937927000000</Timestamp></NotificationRequest></FPolicy>`)
	...
	if len(events) != 1 || events[0].Inode != 1_048_576 || events[0].Vserver != "svm-50" || events[0].VolumeUUID != "98765432-abcd-ef01-2345-6789abcdef01" {
